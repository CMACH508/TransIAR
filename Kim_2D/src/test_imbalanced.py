...
X_nouse = torch.tensor(np.array(dataset_nouse['X_nouse']))
y_test = torch.tensor(dataset_split['y_test'], dtype=torch.long)
y_nouse = torch.tensor(dataset_nouse['y_nouse'], dtype=torch.long)
X_test = torch.cat((X_test, X_nouse))
y_test = torch.cat((y_test, y_nouse))
