...
def train(n_epoch=200, BS=args.batch_size, LR=1e-4):
    ...
    X_train = torch.tensor(dataset_train['vox_train'])[:,0:1,:,:,:]
    X_test = torch.tensor(dataset_test['vox_test'])[:,0:1,:,:,:]
    ...
    for epoch in range(n_epoch):
        ...
        for i, (voxel, cls_idx) in enumerate(test_dataloader, 0):
            ...
            # print('pred_choice:', pred_choice.shape, pred_choice)  # torch.Size([]) tensor(1, device='cuda:0')
            # print('cls_idx:', cls_idx.shape, cls_idx[0])  # torch.Size([32]) tensor(1, device='cuda:0')
...
if __name__ == '__main__':
    ...
    train(n_epoch=200, BS=128, LR=1e-4)
