...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.ups = nn.Upsample(size=(224, 224))
        m = OrderedDict()
...
def resnet50(pretrained=False, model_root=None, **kwargs):
    ...
    if pretrained:
        ...
        load_state_dict(model, 'https://download.pytorch.org/models/resnet50-19c8e357.pth', model_root)
    return model

def resnet18(pretrained=False, model_root=None, **kwargs):
    model = ResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        load_state_dict(model, 'https://download.pytorch.org/models/resnet18-5c106cde.pth', model_root)
    return model
...
if __name__ == '__main__':
    ...
    model.to(device)
    input = torch.randn(size=(1152, 3, 48, 48)).to(device)
    output = model(input)
    print(output.shape)