...
def train(n_epoch = 200, BS = 128, LR=1e-4):
    ...
    model = M3T(n_classes=2, input_shape = input_shape)
    print(model)
    ...
    for epoch in range(n_epoch):
        ...
        if test_acc > best_acc:
            ...
            torch.save(model.state_dict(), '../checkpoint/model_sota_{}.pth'.format(test_acc))
            best_acc = test_acc

        if epoch == n_epoch - 1:
            print('Saving..')
            torch.save(model.state_dict(), '../checkpoint/model_last_{}.pth'.format(test_acc))
